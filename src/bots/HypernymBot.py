...
class HypernymBot(BotBase):
    ...
    def __get_distance_to_first_common_hypernym_aux(self, hypernyms1, hypernyms2, word1, word2):
        ## Base case
        if len(hypernyms1.intersection(hypernyms2)) > 0 or not (self.__has_new_hypernyms(hypernyms1) or self.__has_new_hypernyms(hypernyms2)):
        ...
        return self.__get_distance_to_first_common_hypernym_aux(hypernyms1=self.__get_hypernyms_of_synsets(hypernyms1), hypernyms2=self.__get_hypernyms_of_synsets(hypernyms2), word1=word1, word2=word2)

    
    def __has_new_hypernyms(self, hypernyms_set):
        return len(hypernyms_set) != len(self.__get_hypernyms_of_synsets(hypernyms_set))
