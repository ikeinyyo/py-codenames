...
class HypernymBot(BotBase):
    def __init__(self, is_captain=False, is_team_member=False, language="es"):
        ...
        self.__lang = self.__language_converter(language)
    ...
    def give_answer(self, clue):
        clue = str.lower(clue)
        tuple_of_word_with_distance_to_common_hypernym = list(map(lambda word: (
            word, self.__get_distance_to_first_common_hypernym(clue, word)), self._board.get_available_words()))
        tuple_of_word_with_distance_to_common_hypernym.sort(key=lambda w: -(np.sum(w[1]) + 0.001*(
            1/(1+distance.cosine(self.__model.get_word_vector(w[0]), self.__model.get_word_vector(clue))))))
    ...
    def __get_distance_to_first_common_hypernym_aux(self, hypernyms1, hypernyms2, word1, word2):
        ...
        if len(set(hypernyms1).intersection(hypernyms2)) > 0 or not (self.__has_new_hypernyms(hypernyms1) or self.__has_new_hypernyms(hypernyms2)):
            ...
            distance_to_first_word = list(map(lambda x: max(list(map(
                lambda y: self.__get_path_similarity(x, y), wn.synsets(word1, lang=self.__lang)))), hypernyms))
            distance_to_second_word = list(map(lambda x: max(list(map(
                lambda y: self.__get_path_similarity(x, y), wn.synsets(word2, lang=self.__lang)))), hypernyms))
            possible_hypernyms = zip(np.add(
                distance_to_first_word, distance_to_second_word), distance_to_first_word, distance_to_second_word)
            max_value = max(possible_hypernyms,
                            key=itemgetter(0), default=(0, 0, 0))
            return [max_value[1], max_value[2]]

        list_of_hypernyms_first_word = list(set(hypernyms1 + self.__get_hypernyms_of_synsets(hypernyms1)))
        list_of_hypernyms_second_word = list(set(hypernyms2 + self.__get_hypernyms_of_synsets(hypernyms2)))
        return self.__get_distance_to_first_common_hypernym_aux(list_of_hypernyms_first_word, list_of_hypernyms_second_word, word1, word2)
    ...
    def __language_converter(self, language):
